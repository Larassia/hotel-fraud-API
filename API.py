...
# ==========================================
# Quadrant 3: Top-K Prioritization & Excel Export
# ==========================================
with row2_col1:
    ...
    # ---------------------------------------------------------
    # ---------------------------------------------------------
    export_df = scored_data.copy()
    
    def assign_action(prob):
        if prob >= 0.90: return "High-level escalation"
        elif prob >= 0.50: return "Targeted testing"
        elif prob >= 0.10: return "Screening review"
        else: return "Routine documentation"
        
    export_df['audit_action'] = export_df['posterior_fraud_probability'].apply(assign_action)
    export_df['abs_z_residual'] = export_df['z_residual'].abs()
    
    export_df = export_df.rename(columns={'posterior_fraud_probability': 'fraud_probability'})
    
    summary_df = export_df.groupby('audit_action').agg(
        records=('hotel_id', 'count'),
        mean_probability=('fraud_probability', 'mean'),
        max_probability=('fraud_probability', 'max'),
        mean_abs_z=('abs_z_residual', 'mean')
    ).reset_index()
    
    risk_order = {"Routine documentation": 0, "Screening review": 1, "Targeted testing": 2, "High-level escalation": 3}
    summary_df = summary_df.sort_values(by='audit_action', key=lambda x: x.map(risk_order))
    
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        export_df.to_excel(writer, sheet_name='scored_records', index=False)
        summary_df.to_excel(writer, sheet_name='action_summary', index=False)
    excel_data = output.getvalue()
    
    st.download_button(
        label="📥 下载完整审计评分与分级结果 (Excel)",
        data=excel_data,
        file_name="hotel_revenue_scored_output.xlsx",
